...
private:
	...
	void call_entry(asCScriptFunction* sFunc);
	void call_error();
	void call_exit();
};
...
int asCJITCompiler::CompileFunction(asIScriptFunction *function, asJITFunction *output) {
	...
	*output = activePage->getFunctionPointer<asJITFunction>();
	...
	assembler::byte* byteStart = (assembler::byte*)cpu.op;
	...
	//Function return {
	volatile assembler::byte* ret_pos = cpu.op;
	...
	volatile assembler::byte* script_ret = 0;
	...
	auto check_space = [&](unsigned bytes) {
		...
		if(remaining < bytes + cpu.jumpSpace) {
			...
			pages.insert(std::pair<asJITFunction,assembler::CodePage*>(*output,activePage));
			byteStart = (assembler::byte*)cpu.op;
		}
	};
...
#define pushPrep(use) \
	...
	reservedPushBytes -= use;
		...
		case asBC_PGA:
			...
			/*{
				asUINT length;
				int r = ((asCScriptEngine*)function->GetEngine())->stringFactory->GetRawStringData((void*)asBC_PTRARG(pOp), NULL, &length);
				if ( r >= 0 )
				{
					char *pszData = new char[length] { NULL };
					((asCScriptEngine*)function->GetEngine())->stringFactory->GetRawStringData((void*)asBC_PTRARG(pOp), pszData, &length);
					const void *pvData = ((asCScriptEngine*)function->GetEngine())->stringFactory->GetStringConstant(pszData, length);
					delete pszData;

					if ( pvData != NULL )
					{
						as<void*>(*esi + reservedPushBytes) = const_cast<void*>(pvData);
						break;
					}
				}
			}*/
		...
		case asBC_NOT:
			{
				if(currentEAX != EAX_Offset + offset0)
					al = as<assembler::byte>(*edi-offset0);
				al &= al;
				al.setIf(Zero);
				eax.copy_zeroing(al);
				*edi-offset0 = eax;
		...
		case asBC_SUSPEND:
			if(flags & JIT_NO_SUSPEND) {
				...
				cl = as<assembler::byte>(*ebp+offsetof(asSVMRegisters,doProcessSuspend));
				cl &= cl;
...
#define cast(f,t) {\
	...
	constexpr bool equalSize = sizeof(f) == sizeof(t);\
	if(!equalSize)\
...
void SystemCall::call_entry(asCScriptFunction* sFunc) {
	unsigned pBits = sizeof(void*) * 8;
#ifdef JIT_64
